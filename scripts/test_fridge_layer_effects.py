...
def layer_difference_test_img_feature(criteria=criteria):
    ...
    for criterion in criteria:
        ...
        plt.legend()
        plt.savefig(f'plots/max_mean_dif_test_{criterion}_unbagged.png')
        plt.close()
    

def layer_difference_test_weight():
    ...
    plt.axvline(x=output['max_mean_dif_observe'], label='Observed', color='salmon')
    plt.legend()
    plt.savefig(f'plots/max_mean_dif_test_weights_unbagged.png')
    plt.close()
